...
@asset_management_router.post("/stocks", summary="주식 현재가를 반환합니다.", response_model=list[StockResponse])
async def get_stocks(
    stock_codes: list[str], db: Session = Depends(get_mysql_session), token: str = Depends(verify_jwt_token)
) -> list[StockResponse]:
    if stock_codes is []:
