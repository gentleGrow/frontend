...
class TestDividendFacade:
    async def test_get_composition(self, setup_all, session: AsyncSession, redis_client: Redis):
        # Given
        ...
        expected_result = [
            ...
            (
                "TSLA",
                approx(0.8 * 1300 * 2),
                approx((0.8 * 1300 * 2) / (1.5 * 1300 + (0.8 * 1300 * 2) + 100.0 * 1.0) * 100),
            ),
            (
                "005930",
                approx(100.0 * 1.0),
                approx((100.0 * 1.0) / (1.5 * 1300 + (0.8 * 1300 * 2) + 100.0 * 1.0) * 100),
            ),
        ]

        expected_result = sorted(expected_result, key=lambda x: x[0])

        for (stock_code, dividend, percentage), (expected_stock_code, expected_dividend, expected_percentage) in zip(
