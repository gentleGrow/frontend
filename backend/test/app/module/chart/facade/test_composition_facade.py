...
class TestCompositionFacade:
    async def test_get_asset_stock_composition(
        ...
        current_stock_price_map = {"AAPL": 150.0, "TSLA": 720.0, "005930": 70000.0}
        ...
        result = CompositionFacade.get_asset_stock_composition(
            assets=assets, current_stock_price_map=current_stock_price_map, exchange_rate_map=exchange_rate_map
        )
        ...
        expected_result = [
            {
                ...
                "percent_rate": approx(
                    (2 * 720 * exchange_rate_map["USD_KRW"])
                    / (1 * 150 * exchange_rate_map["USD_KRW"] + 2 * 720 * exchange_rate_map["USD_KRW"] + 1 * 70000)
                    * 100,
                    rel=1e-2,
                ),
                "current_amount": approx(2 * 720 * exchange_rate_map["USD_KRW"], rel=1e-2),
