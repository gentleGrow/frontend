...
class TestStockDailyRepository:
    async def test_get_latest(self, db_session: AsyncSession, setup_stock_daily):
        # Given
        stock_codes = ["AAPL", "TSLA"]

        # When
        latest_stock_dailies = await StockDailyRepository.get_latest(db_session, stock_codes)
    ...
    async def test_get_stock_dailies_by_code_and_date(self, db_session: AsyncSession, setup_stock_daily) -> None:
        # Given
        setup_stock_daily
        ...
        # When
        stock_dailies = await StockDailyRepository.get_stock_dailies_by_code_and_date(db_session, stock_code_date_pairs)

        # Then
        assert len(stock_dailies) == 4

        # Check the first stock (AAPL, 2024-08-13)
        ...
        # Check the second stock (AAPL, 2024-08-14)
        ...
        # Check the third stock (TSLA, 2024-08-13)
        ...
        # Check the fourth stock (TSLA, 2024-08-14)
